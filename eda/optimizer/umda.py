...
class UMDA(EDABase):
    ...
    def update(self, x, evals, range_restriction=False):
        ...
        # print(" self.theta[:, -1]", self.theta[:, -1])
        # print("np.mean " ,np.mean(x[:, :, -1], axis=0))
        # print(" resta", (np.mean(x[:, :, -1], axis=0) - self.theta[:, -1]))
        # print(x)
        # print("evals", np.mean(x[:, :, -1], axis=0))
        # print(np.mean(x[:, :, -1], axis=0))
        self.theta[:, -1] += self.lr * (np.mean(x[:, :, -1], axis=0) - self.theta[:, -1])
        self.theta[:, 0] = 1 - self.theta[:, -1]
        #print(" prob -1", self.theta)
